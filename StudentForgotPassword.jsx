...
const StudentForgotPassword = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault()
        if(!isDetailSub){
            const isValidDomain = email.endsWith('@srmap.edu.in');
            if (!isValidDomain) {
                toast.error("Invalid email, please enter srm email Id")
                return;
            }
        ...
        <div className="fp-box">
            {
                isOtpSub ? (
                    ...
                    <form className="fp-form" onSubmit={handleSubmit}>
                        ...
                        <input type="text" name="enroll" id='enroll' className="fp-inp fp-up" placeholder='eg. AP22110010196' onChange={(e) => {setEnrollment(e.target.value.toUpperCase())}} required/>
                        <label htmlFor="email" className="fp-label">Enter your Email Id:</label>
                        <input type="email" name='email' id='email' className="fp-inp" placeholder='eg.example@srmap.edu.in' onChange={(e) => {setEmail(e.target.value)}} required/>
                        {isDetailSub &&
                            <>
